...
class RestaurantBot extends ActivityHandler {
  ...
  constructor() {
    ...
    this.onMessage(async (context, next) => {
      ...
      let time24hr = "19:00:00"; // Default 7 PM
      ...
      switch (topIntent) {
        case "GeneralGreeting": {
          switch (userProfile.generalGreetingState) {
            case undefined: {
              ...
              reply =
                "👋 Hello! Welcome to our Restaurant Bot.\n\n" +
            ...
            default: {
              delete userProfile.generalGreetingState;
              reply =
        ...
        case "AddItemToCart": {
          try {
            ...
            if (!item) {
              reply = `❌ Sorry, I couldn't find **${mItem}** in our menu. Please try another item.`;
              break;
            }
            if (!userProfile.addItemState) {
              userProfile.currentItem = mItem;
              userProfile.addItemState = "awaiting_quantity";
              reply = `🍴 You'd like to add **${mItem}**. How many would you like to order?`;
              break;
              ...
              if (!quantity || quantity <= 0) {
                ...
                const added = await addItemToUserCart(
                  userProfile.userId,
                  item.id,
                  quantity
                );
                if (added) {
                  reply = `✅ I've added ${quantity} x **${mItem}** to your cart.\n\nWhat would you like to do next?\n\n• 🛍️ Add more items\n• 🧾 View your cart\n• ✅ Proceed to checkout\n• ❌ Remove an item`;
                } else {
                  reply =
                    "❌ Could not add the item to your cart. Please try again later.";
                }
        ...
        case "PlaceOrder": {
          try {
            ...
            if (!userProfile.orderConfirmationState) {
              ...
              if (text?.toLowerCase() === "yes") {
                const placed = await placeOrder(rName, userProfile.userId, [
                  { name: mItem, quantity },
                ]);
                if (placed) {
